...
public abstract class Response
{
    ...
    public static async Task<Response> ReadAsync(Stream stream)
    {
        ...
        {
            ResponseType.UpdateCompleted => UpdateCompletedResponse.Create(reader),
...
/// 
/// </summary>
public class UpdateCompletedResponse : Response
{
    public int Changes { get; }
    public string Version { get; }

    public override ResponseType Type => ResponseType.UpdateCompleted;

    public UpdateCompletedResponse(int changes, string version)
    {
        Changes = changes;
        Version = version;
    }

    public static UpdateCompletedResponse Create(BinaryReader reader)
    {
        var changes = reader.ReadInt32();
        var version = ReadLengthPrefixedString(reader);
        return new UpdateCompletedResponse(changes, version);
    }
}
